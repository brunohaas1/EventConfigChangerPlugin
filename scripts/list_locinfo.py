...
def list_methods(name):
    ...
    for _ in range(nm):
        ...
        for _ in range(nattr):
            alen = struct.unpack('>I', data[pos+2:pos+6])[0]; pos += 6 + alen
        m = utf(cp, name_idx); d = utf(cp, desc_idx)
        if m.startswith('get') or m in ('click', 'hover', 'setX', 'setY'):
            print('   ', m + d)

for n in ['com/github/manolo8/darkbot/core/objects/LocationInfo.class',
          'com/github/manolo8/darkbot/core/objects/SpriteObject.class']:
    list_methods(n)