...
def parse_cp(data):
    ...
    while i < count:
        ...
        try:
            ...
            elif tag in (21, 22):  # Module/Package (Java 9+)
                cp.append(data[idx:idx+2]); idx += 2
            else:
                # Unknown tag: skip 2 bytes as a safe fallback
                cp.append(b''); idx += 2
        except Exception:
            cp.append(b''); idx += 2
        i += 1
...
def disasm(code, cp):
    ...
    while pc < len(bc):
        ...
        if op == 0xb4:  # getfield
            ...
            pc += 3
        elif op == 0xb6 or op == 0xb7:  # invokevirtual/invokespecial
            ref = struct.unpack('>H', bc[pc+1:pc+3])[0]
...
for n in ['com/github/manolo8/darkbot/core/objects/Gui.class']:
    ...
    print('===', n)
    for tname, tdesc in [('click', '(II)V'), ('hover', '(II)V'),
                         ('getX', '()D'), ('getY', '()D'),
                         ('getX2', '()D'), ('getY2', '()D')]:
